package aiLibsExperiments.small.mlplan;
...
public class MLPlan_small_Experimenter implements IExperimentSetEvaluator {
	...
	public void evaluate(ExperimentDBEntry experimentEntry, SQLAdapter adapter,
		...
		File tempFile = File.createTempFile("tmp-mlplan-"+experimentEntry.getId(), "json");
		...
		tempFile = File.createTempFile("tmp-mlplan-"+experimentEntry.getId(), "");
		tempFile.deleteOnExit();
		out = new FileOutputStream(tempFile);
		in = Thread.currentThread().getContextClassLoader().getResourceAsStream("mlplan.properties");
		IOUtils.copy(in, out);
		File algorithmConfigFile = tempFile;
		
		MLPlanWekaBuilder builder = new MLPlanWekaBuilder(
				configFile, algorithmConfigFile,
